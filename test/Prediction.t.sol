...
contract PredictionTokenTest is Test {
    ...
    function test_GasReport_approveForSub() public {
        ...
        // Set general approval first
        vm.prank(trader1);
        _token().approve(trader2, 1000e18);

        // Do 5 approveForSub calls
        for (uint i = 0; i < 10; i++) {
            vm.prank(trader1);
    ...
    function test_GasReport_allowanceOf() public {
        ...
        for (uint i = 0; i < 10; i++) {
            (uint al, , ) = _token().allowanceOf(trader1, 1, trader2);
            al += 1;
        }
    }
    ...
    function test_GasReport_transfer_() public {
        _warpToActiveRound(ROUND);
        _mint(trader1, 2000e18, 70000e8, ROUND);
        vm.prank(trader1);
        _token().convertToSuper();

        // Set permission on receiver with wide range to avoid failure
        vm.prank(trader2);
        _token().permitForSub(0, 0, true, 65000e8, 95000e8, false);

        vm.prank(trader1);
        _token().transfer(trader2, 1000e18);

        // Mint another 1000e18 with a different prediction
        _mint(trader1, 1000e18, 90000e8, ROUND);
    ...
    function test_GasReport_transferFrom() public {
        ...
        for (uint i = 0; i < 12; i++) {
            vm.prank(trader2);
            _token().transferFrom(trader1, trader2, amount);
        }
    }
    ...
    function test_GasReport_parametricTransfer_() public {
        ...
        // Set permission on sub-account 1
        vm.prank(trader1);
        _token().permitForSub(1, 0, true, 65000e8, 95000e8, false);

        vm.prank(trader1);
        _token().parametricTransfer(0, trader1, 1, 1000e18);
        ...
        for (uint i = 0; i < 12; i++) {
            vm.prank(trader1);
            _token().parametricTransfer(0, trader1, 1, amount);
        }
    }
    ...
    function test_GasReport_parametricTransferFrom() public {
        ...
        _token().convertToSuper();
        _token().createSubAccount();
        _token().parametricTransfer(0, trader1, 1, 1500e18);
        _token().approveForSub(1, trader2, 2000e18, false, 0);
        vm.stopPrank();

        // Set permission on receiver's sub-account 0
        vm.prank(trader2);
        _token().permitForSub(0, 0, true, 65000e8, 95000e8, false);

        vm.prank(trader1);
        _token().parametricTransfer(1, trader2, 0, 1000e18);
    ...
    function test_GasReport_mint() public {
        _warpToActiveRound(ROUND);
        _mint(trader1, 1000e18, 70000e8, ROUND);
        // Mint with super account and permissions already set
        vm.prank(trader1);
        _token().convertToSuper();

        // Set permission on sub-account 0 for minting
        vm.prank(trader1);
        _token().permitForSub(0, 0, true, 65000e8, 95000e8, false);
        ...
        for (uint i = 0; i < 12; i++) {
            vm.prank(trader1);
            _token().mint(trader1, amount, 90000e8, ROUND);
        }
    }
    ...
    function test_GasReport_burn() public {
        _warpToActiveRound(ROUND);
        _mint(trader1, 1000e18, 70000e8, ROUND);
        // Convert to super and create sub-account to add gas
        vm.startPrank(trader1);
        _token().convertToSuper();
        _token().createSubAccount();
        vm.stopPrank();
        ...
        for (uint i = 0; i < 12; i++) {
            vm.prank(address(engine));
            _token().burn(trader1, 0, amount);
        }
    }

    function test_GasReport_permitForSub() public {
        _warpToActiveRound(ROUND);
        _mint(trader1, 1000e18, 70000e8, ROUND);
        vm.startPrank(trader1);
        _token().convertToSuper();
        ...
        // Do 5 permitForSub calls, enabling/disabling each time to keep state fresh
        for (uint i = 0; i < 12; i++) {
            vm.prank(trader1);
            _token().permitForSub(
                1,
                0,
                true,
                65000e8 + uint64(i * 1000e8),
                75000e8 + uint64(i * 1000e8),
                false
            );
        }
    }

    function test_GasReport_permissionOf() public {
        _warpToActiveRound(ROUND);
        vm.startPrank(trader1);
        _token().convertToSuper();
        _token().permitForSub(0, 0, true, 65000e8, 75000e8, false);
        ...
        for (uint i = 0; i < 12; i++) {
            _token().permissionOf(trader1, 0, 0);
        }
    }
}
