...
contract TenureTokenTest is Test {
    ...
    function test_GasReport_transfer_() public {
        // Mint enough to keep balance >0 during loop
        _mint(trader1, 2000e18);
        vm.startPrank(trader1);
        _token().convertToSuper();
        ...
        _mint(trader1, 1000e18);
        // Now prediction (mintTime) is weighted average of two mint times
    ...
    function test_GasReport_parametricTransfer_() public {
        ...
        for (uint i = 0; i < 12; i++) {
            vm.warp(block.timestamp + 20000);
            vm.prank(trader1);
            _token().parametricTransfer(1, trader2, 0, amount);
        }
    }
    ...
    function test_GasReport_parametricTransferFrom() public {
        ...
        _token().convertToSuper();
        _token().createSubAccount();
        _token().transfer(trader2, 1500e18);
