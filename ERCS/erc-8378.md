...
- If `enabled == true`, inbound transfer's parameter has to meet limits set by respective pair of `min` and `max`; any transfers violating this MUST revert.
- If `enabled == true`, `soft` MUST be `false`; any call violating this MUST revert.
...
- `allPermissionsOf(address account, uint48 subId)` returns an array of all active indexed permissions; all permission records with `enabled == false` MUST be omitted.
