...
- `approveForSub(uint48 ownerSubId, address spender, uint256 amount, bool oneOff)`:
  ...
  - If `ownerSubId == 0` (i.e., Default sub-account): `amount` MUST be `0` and `oneOff` MUST be `false`. Resulting state implies the account has no Specific allowance.
...
- The standard introduces additional implementation risks arising from parameter state, deterministic transitions, sub-accounts, and optional non-zero-sum accounting. These risks are distinct from, and in addition to, standard ERC-20 risks such as allowance misuse and reentrancy.
